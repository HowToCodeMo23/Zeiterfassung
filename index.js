...
app.use(
  session({
    secret: "testtest", // Geheimer Schlüssel zur Sitzungsverschlüsselung
...
app.post("/time", async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount > 0) {
      ...
      console.error("Keine Zeiten eingefügt.");
      res.redirect("/dashboard");
    }
  } catch (error) {
...
app.get("/get_events", async (req, res) => {
  if (req.session.user) {
    try {
      const startDate = req.query.startDate + " 06:00:00";
      const endDate = req.query.endDate + " 22:30:00";
      let today = new Date(req.query.startDate);
      ...
      // Abfrage der Zeitdaten und der Projekte
      const timesResult = await db.query(
        `
                SELECT times.startzeit as startzeit, times.endzeit as endzeit, times.last_updated as last_updated, projects.projectname as projectname
                FROM times
                INNER JOIN projects ON times.p_id = projects.projectid
                WHERE times.e_id = $1 AND times.startzeit >= $2 AND times.endzeit <= $3 AND DATE(times.last_updated) = $4
                `,
        [req.session.user.personalid, startDate, endDate, today]
      );

      if (timesResult.rows.length > 0) {
        console.log("timesResult.rows: ", timesResult.rows);

        const events = timesResult.rows.map((time) => {
          console.log("projectname: ", time.projectname);

          const occasion = " Zeiteintrag ";
          const [startYear, startMonth, startDay] = time.last_updated
            .toISOString()
            .split("T")[0]
            .split("-");
          const startMinutes = time.startzeit.split(":");
          const startHour = parseInt(startMinutes[0]) + 2;
          const startMinute = startMinutes[1];
          const [endYear, endMonth, endDay] = time.last_updated
            .toISOString()
            .split("T")[0]
            .split("-");
          const endMinutes = time.endzeit.split(":");
          const endHour = parseInt(endMinutes[0]) + 2;
          const endMinute = endMinutes[1];
